...
function addEvents(movieLists, creditLists) {
  ...
  sortButton1?.addEventListener("click", (event) => {
    ...
    listIDs.forEach((listID, index) => sortByTitle(movieLists[index], creditLists[index], listID, prevSortKey));
...
async function initialize() {
  ...
  listIDs.forEach((listID, index) => {
    createCardsInList(movieLists[index], listID, creditLists[index]);
  });
